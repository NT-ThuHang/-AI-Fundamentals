...
from search_algorithm import BFS, example_func, DFS, AStar, UCS, GreedySearch, BestFirstSearch
...
def run(graph, start, goal, search_alg=None):
    ...
    updateUI()
    pygame.time.delay(delay_start_search_time+100)  # wait 5 sec to start
    ...
    elif search_alg == 'GreedySearch':
        GreedySearch(global_graph, edges, edge_id, start, goal)
    elif search_alg == 'BestFirstSearch':
        BestFirstSearch(global_graph, edges, edge_id, start, goal)
